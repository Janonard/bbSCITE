...
\begin{lemma}
    ...
    Let $A$, $B$ and $C$ be the classes from definition \ref{def:related_swap_classes}, $x \in A$, and $y \in B \cup C$. We have $x \leadsto_{T'} y \Leftrightarrow x \leadsto_T y$.
\end{lemma}
...
\begin{lemma}
    \label{lem:related_swap_ac}
    Let $A$ and $C$ be the classes from definition \ref{def:related_swap_classes}, $x \in A$, and $y \in C$. We have $`x \leadsto_{T'} y \Leftrightarrow x \leadsto_T y`$.
\end{lemma}

\begin{proof}
    The proof is analogous to lemma \ref{lem:related_swap_ab}, only that the middle section in the path is removed by the swap operation, not added.
\end{proof}

\begin{lemma}
    \label{lem:related_swap_bac}
    If $x \in B$ and $y \in A$  or $y \in C$, we have $x \not\leadsto_{T'} y$, since every possible link would be broken (to be extended, but obvious with the diagram).
\end{lemma}
...
\begin{proof}
    Proof of $\Rightarrow$. We have $y \in B \Rightarrow w \leadsto_{T'} y \Rightarrow t \leadsto_{T'} y$. Since the path from $x$ has to go via $t$ and we have $t \in C$, it follows that $x \leadsto_T t$.

    Proof of $\Leftarrow$. Since we have $y \in B \Rightarrow w \leadsto_{T'} y \Rightarrow t \leadsto_{T'} y$  and $x \in C \Rightarrow x \leadsto_{T'} t$, it follows that $x \leadsto_{T'} t \leadsto_{T'} y$.
\end{proof}
