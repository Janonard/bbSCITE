...
\begin{figure}
    \centering
    \begin{tikzpicture}
        \node (r) {$r$}[level distance=1cm] child {
            node (pw) {$p_T(w)$}[level distance=2cm] child [->] {
                node (w) {$w$}[level distance=1cm] child {
                    node (pv) {$p_T(v)$}[level distance=2cm] child {
                        node (v) {$v$}[level distance=1cm] child {
                            node (t) {$t$}
        ...
        \draw[thick] ($(r) + (0,1)$) -- ($(pw) + (-\sinOffset,-\cosOffset)$) -- ($(an) + (\sinOffset,-\cosOffset)$) -- cycle;
        \draw[thick] ($(w) + (0,1)$) -- ($(pv) + (-\sinOffset,-\cosOffset)$) -- ($(bn) + (\sinOffset,-\cosOffset)$) -- cycle;
        \draw[thick] ($(v) + (0,1)$) -- ($(t) + (-\sinOffset,-\cosOffset)$) -- ($(cn) + (\sinOffset,-\cosOffset)$) -- cycle;

        \node[thick] at ($(r) + (-1,0)$) {$A$};
        \node[thick] at ($(w) + (-1,0)$) {$B$};
        \node[thick] at ($(v) + (-1,0)$) {$C$};
    \end{tikzpicture}
    \caption{Illustration of the different classes from definitions \ref{def:related_swap_classes} before the move. Squiggly lines indicate $\leadsto_T$ relationships, while straight lines indicate proper edges.}
