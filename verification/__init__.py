...
def random_mutation_tree(n_cells, n_genes):
...
def to_newick_code(tree: nx.Graph, parent: int):
    if tree.out_degree(parent) > 0:
        return f"({','.join(to_newick_code(tree, node) for node in tree.adj[parent])}){parent}"
    else:
        return f"{parent}"
...
def score_mutation_matrix(mutation_tree: MutationTree, mutation_matrix: np.array, prob_false_positives: float, prob_false_negatives: float):
    true_mutation_matrix = mutation_tree.get_mutation_matrix()
    assert mutation_matrix.shape == (
        mutation_tree.n_genes, mutation_tree.n_cells)

    occurrences = [[0, 0], [0, 0]]
    for gene_i in range(mutation_tree.n_genes):
        for cell_i in range(mutation_tree.n_cells):
    ...
    likelihood = (1.0 - prob_false_positives)**occurrences[0][0]
    likelihood *= prob_false_positives**occurrences[0][1]
    likelihood *= prob_false_negatives**occurrences[1][0]
    likelihood *= (1.0 - prob_false_negatives)**occurrences[1][1]
    return likelihood
